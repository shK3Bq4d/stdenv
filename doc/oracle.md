...
SELECT dbms_metadata.get_ddl('PROCEDURE',upper('UsafeAcquisiteurs'), 'CONT') into v_str  FROM DUAL;
...
select flux.seq_facnbcleflux.nextval from dual connect by level <= 100; -- for a 100 upgrade
select flux.seq_facnbcleflux.nextval from dual connect by level <= (select max(nbcleflux) from sini.mcddocument);
...
SELECT WM_CONCAT(tx_fileid) FROM flux.facsuivi where rownum < 5; 
...
SELECT EXTRACT(XMLTYPE('<doc>' || XMLAGG(XMLTYPE('<ln>' || tx_fileid || ','|| CHR(10) || '</ln>')).GetClobVal() || '</doc>'), '/doc/ln/text()').GetClobVal()   FROM flux.facsuivi where rownum < 5; 
